...
function getRating(watchList) {
  ...
  const nolanMovies = watchList.filter(film => film.Director === "Christopher Nolan");

  // Step 2: Extract IMDb ratings
  const ratings = nolanMovies.map(film => Number(film.imdbRating));

  // Step 3: Calculate the sum of ratings
  const totalRating = ratings.reduce((sum, rating) => sum + rating, 0);

  // Step 4: Calculate the average
  const avgRating = totalRating / nolanMovies.length;
